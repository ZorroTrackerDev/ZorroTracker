...
export class FileApi {
	...
	private static _cachedFileMappings: { [key: string]: string|string[] };
	 ...
	 */
	public static loadSettingsFiles(settingsType:SettingsTypes): unknown[] {
		if(!FileApi._cachedFileMappings) {
			// if the configuration file was not cached, cache it now.
			const rawfile = fs.readFileSync(fileMappingsSource, "utf-8");
			FileApi._cachedFileMappings = json5.parse(rawfile);
		}

		// check if the settings entry exists. If not, pretend we have got 0 files
		if(!FileApi._cachedFileMappings[settingsType]){
			return [];
		}
		...
		if(Array.isArray(FileApi._cachedFileMappings[settingsType])) {
			// this is an array of filesnames, convert every one individually
			(FileApi._cachedFileMappings[settingsType] as string[]).forEach(convertSingleFile);

		} else {
			// this is not an array, convert the single filename
			convertSingleFile(FileApi._cachedFileMappings[settingsType] as string);
		}
